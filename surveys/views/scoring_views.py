...
@login_required
def enhanced_calculate_score(request, questionnaire_id, response_id):
    """
    Calculate enhanced score for a response using the EnhancedScoringService

    This view calculates additional metrics including:
    - Z-score
    - Percentile
    - Category scores
    - Subscale scores
    ...
    if not request.user.is_staff and questionnaire.created_by != request.user:
        if questionnaire.organization:
            # Check if user is a member of the organization
            if not questionnaire.organization.members.filter(user=request.user, is_active=True).exists():
                messages.error(request, "You don't have permission to calculate enhanced scores for this response.")
                return redirect('surveys:survey_detail', pk=questionnaire_id)
        else:
            messages.error(request, "You don't have permission to calculate enhanced scores for this response.")
            return redirect('surveys:survey_detail', pk=questionnaire_id)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.success(
                request,
                f"Enhanced score calculated successfully: {response_score.raw_score} "
                f"(Z-score: {response_score.z_score}, Percentile: {response_score.percentile}%)"
            )
            return redirect('feedback:response_detail', pk=response_id)

        except Exception as e:
            messages.error(request, f"Error calculating enhanced score: {str(e)}")
            return redirect('feedback:response_detail', pk=response_id)
